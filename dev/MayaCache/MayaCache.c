...
static int infosSize;
 ...
 void enableChannel(CHANNELTYPE channelActive, ENABLEDISABLED ea, int cacheId)
 {
	 ...
	 else
	 {
		...
		infos[index].mayaChannels[channelActive].type = FVCA;
		if (infos[index].mayaChannels[channelActive].elementsD != NULL)
			free(infos[index].mayaChannels[channelActive].elementsD);
		if (infos[index].mayaChannels[channelActive].elementsF != NULL)
			free(infos[index].mayaChannels[channelActive].elementsF);
		if (infos[index].mayaChannels[channelActive].name != NULL)
			free(infos[index].mayaChannels[channelActive].name);
		infos[index].mayaChannels[channelActive].elementsD = NULL;
		infos[index].mayaChannels[channelActive].elementsF = NULL;
		infos[index].mayaChannels[channelActive].name = NULL;
	 }
}
 ...
 int init(char *particleSysName,char *fileName, CACHEFORMAT cacheFormat,int nExtraChannels, unsigned int fps, double start, double end,char *extra[], int nE)
 {
	char *filePath;
	char *fileName2;
	char *fileExtension;
	...
	infos[infosSize - 1].mcFileName = (char *)malloc(sizeof(char) * (strlen(fileName) + 3));
	strcpy(infos[infosSize - 1].mcFileName, mcName);
	...
	infos[infosSize - 1].xmlFileName = (char*)malloc( sizeof(char) * ( strlen( fileName ) + XMLEXTENSIONLENGTH + 1 ) );
	strcpy(infos[infosSize - 1].xmlFileName, fileName);
	strcat(infos[infosSize - 1].xmlFileName, XMLEXTENSION);
...
// channel
void writeMayaCacheChannel(CHANNELTYPE cType, int cacheId, int numberOfElements)
{
	...
	else
	{
		...
		else
		{
			...
			channelArrayLength = 8 * numberOfElements;
		}
	}
...
// data block
void writeMayaCacheBlock(int cacheId, int numberOfElements)
{
	...
	for(i = 0; i < CACHENUMBEROFCHANNELS; i++)
		if (infos[index].mayaChannels[i].enabled)
			writeMayaCacheChannel(i,cacheId, numberOfElements);
...
void mayaCache(int cacheId, int numberOfElements)
{
	char *mcName;
	char *subfix;
	int sflenght, fps;
	...
	if (infos[index].cacheFormat == ONEFILE)
		writeMayaCacheBlock(cacheId, numberOfElements);
	else
	{
...
void makeName(CHANNELTYPE type, char **channelName, int cacheId)
{
	...
	*channelName = (char*)calloc(strlen(infos[index].particleSysName) + dim, sizeof(char));
