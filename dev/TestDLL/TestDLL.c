...
int main()
{
	...
	cache1 = init("nParticleShape1", "d://temp//experimental//ONEFILE", ONEFILE, 0, fps, start, end, extras, nExtras);
	cache2 = init("nParticleShape2", "d://temp//experimental//TestSenzaVelocity", ONEFILEPERFRAME, 0, fps, start, end, extras, nExtras);
	...
	for( j = getStartFrame(cache1); j <= getStartFrame(cache1) + getDuration(cache1); j += getFrameIncrement(cache1))
	{
		...
		// chaching simulation
		mayaCache(cache2, numberOfElements);
		mayaCache(cache1, numberOfElements);
